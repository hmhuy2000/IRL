...
class RolloutBuffer_PPO_lag:
    ...
    def append(self, state, action, reward,cost, done, log_pi, next_state):
        self.states[self._p].copy_(torch.from_numpy(state))
        self.actions[self._p].copy_(torch.from_numpy(action))
        self.rewards[self._p] = float(reward)
...
class RolloutBufferwithCost_WCSAC:
    ...
    def append(self, state, action, reward,cost, done, next_state):
        ...
        self.rewards[self._p] = float(reward)
        self.costs[self._p] = float(cost)
        self.not_dones[self._p] = 1 - float(done)
        self.next_states[self._p].copy_(torch.from_numpy(next_state))
